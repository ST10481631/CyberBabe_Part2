...
namespace CyberBabe_Part2
{
    class ChatBot
    {
        public ChatBot(ArrayList reply, ArrayList ignore)
        ...
        {//start of method
            ...
            //password responses
            add_answers.Add("password A password is a secret word or a bunch of letters or numbers used to log into a system");
            add_answers.Add("password A strong password should have at least 8 characters, including uppercase, lowercase, numbers and symbols");

            //phishing responses
            add_answers.Add("phishing Phishing is when someone pretends to be a trusted person who then tricks you into giving your personal information");
            add_answers.Add("phishing Phishing emails often contain urgent messages asking you to click a link or provide your details");

            //safe browsing responses
            add_answers.Add("safe browsing Safe browsing means using the internet carefully when visiting websites");
            add_answers.Add("safe browsing Do not click on links you do not know or trust");

            //malware responses
            add_answers.Add("malware Malware is malicious software designed to damage or gain unauthorised access to a computer system");
            add_answers.Add("malware You can protect yourself from malware by keeping your antivirus software up to date");

            //firewall responses
            add_answers.Add("firewall A firewall is a security system that monitors and controls incoming and outgoing network traffic");
            add_answers.Add("firewall Firewalls act as a barrier between your trusted internal network and untrusted external networks");

            //two factor authentication responses
            add_answers.Add("two factor authentication Adds an extra layer of security by requiring a second form of verification");
            add_answers.Add("two factor authentication Two factor authentication means even if someone steals your password they still cannot access your account");

            //encryption responses
            add_answers.Add("encryption Is the process of changing data into a coded format that can only be read by people who are authorised authorised");
            add_answers.Add("encryption Protects sensitive data like banking details and personal information from being stolen");

            //data breach responses
            add_answers.Add("data breach A data breach is when people who are unauthorised individuals get access to data that is confidential");
            add_answers.Add("data breach If your data is breached you should immediately change your passwords and check your accounts for any activity that is suspicious");
        ...
        public string getResponse(string input, ArrayList replies)
        {
            ...
            foreach (string item in replies)
            {
                ...
                if (parts.Length > 0)
                {
                    ...
                    {
                        return item.Substring(keyword.Length).Trim();
                    }
                }
            }

            return "I am not sure about that, but I can help with cybersecurity topics like phishing, malware, and passwords.";
        }
